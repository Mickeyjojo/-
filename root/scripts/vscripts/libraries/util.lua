--[[
	...
	GetClosestUnit(point,unitGroup)
	...
	GetBehaviorTable(behavior)
	GetRandomValueInTable(table)
...
function GetClosestUnit( ... )
	...
	for k,v in pairs(unitGroup) do
		if v ~= caster then
			local distance = (point - v:GetAbsOrigin()):Length2D()
...
function GetBehaviorTable( ... )
	...
	local n = 0
	while behavior > 0 do
		while 2 ^ n <= behavior do
			n = n + 1
		end
		behavior = behavior - 2 ^ (n - 1)
		table.insert( behaviorTable, 2 ^ (n - 1) )
		n = 0
	end
	return behaviorTable
end
-- 选取表中的随机值
function GetRandomValueInTable(Table)
    local tmpKeyT = {}
    local n = 1
    ...
    return Table[tmpKeyT[RandomInt(1, n - 1)]]
end
-- 选取范围内随机点
